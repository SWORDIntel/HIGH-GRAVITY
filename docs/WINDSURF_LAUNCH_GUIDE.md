...
**Basic launch:**
```bash
bin/gemini_session_launcher.py --mode windsurf --provider proxy --proxy-url http://localhost:9999
```

**With specific key:**
```bash
bin/gemini_session_launcher.py --mode windsurf --provider proxy --proxy-url http://localhost:9999 --key-index 1
```

**With custom window name:**
```bash
bin/gemini_session_launcher.py --mode windsurf --provider proxy --proxy-url http://localhost:9999 --window-name my-project
```
...
### Method 1: Echo JSON
```bash
echo '{"mode":"windsurf","provider":"proxy","proxyUrl":"http://localhost:9999"}' | bin/launch_windsurf_piped.sh
```
...
| `--proxy-url URL` | Proxy endpoint | `http://localhost:9999` |
...
### With Key Index
```bash
echo '{"mode":"windsurf","provider":"proxy","proxyUrl":"http://localhost:9999","keyIndex":"1"}' | bin/launch_windsurf_piped.sh
```

### With Custom Window Name
```bash
echo '{"mode":"windsurf","provider":"proxy","proxyUrl":"http://localhost:9999","windowName":"my-project","model":"gemini-exp-1206"}' | bin/launch_windsurf_piped.sh
```

### Dry Run (Test Configuration)
```bash
echo '{"mode":"windsurf","provider":"proxy","proxyUrl":"http://localhost:9999","dryRun":true}' | bin/launch_windsurf_piped.sh
```
...
cat << EOF | bin/launch_windsurf_piped.sh
{
  "mode": "windsurf",
  "provider": "proxy",
  "proxyUrl": "${HIGHGRAVITY_PROXY_URL:-http://localhost:9999}",
