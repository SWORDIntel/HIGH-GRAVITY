...
```
Windsurf → Proxy (9998/443) → Codeium API
                ↓
...
```bash
./hg.sh start
# Starts proxy + Khoj + Windsurf
# Khoj starts automatically via Docker
```

### 2. Verify Khoj is Running
```bash
curl -fsS --max-time 3 http://127.0.0.1:42110/api/health >/dev/null && echo healthy
# Health check is based on HTTP 200 (response JSON may vary by version)
```

### 3. Persistent Disk State
By default, Khoj state is persisted on host disk:
- `/tank/khoj` when `/tank` exists
- otherwise `./data/khoj`
...
GET /api/health
```
Healthy is determined by HTTP success (2xx). JSON payload shape can vary by Khoj version.
...
```
GET http://127.0.0.1:9998/hg/khoj/status
```
Returns:
```json
{
...
### 3. Reindex Trigger
```
POST http://127.0.0.1:9998/hg/khoj/reindex
```
...
```bash
curl http://127.0.0.1:9998/hg/telemetry | jq '.khoj'
```
...
- Dashboard: `./hg.sh dashboard` (or `./hg.sh dashboard-v3`)
