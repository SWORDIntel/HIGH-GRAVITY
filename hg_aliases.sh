...
# Dashboard
alias hg="python3 $HG_ROOT/hg.py"
...
# Piped launcher
alias hg-pipe="$HG_ROOT/bin/launch_windsurf_piped.sh"
...
hg-launch() {
    ...
    cat "$HG_ROOT/config/windsurf_launch_examples.json" | jq -r ".examples.$config.config" | $HG_ROOT/bin/launch_windsurf_piped.sh
}

hg-launch-json() {
    echo "$1" | $HG_ROOT/bin/launch_windsurf_piped.sh
}
