...
def test_agg_multiple_functions_same_name():
    ...
    result = df.resample("3min").agg(
        {"A": [partial(np.quantile, q=0.9998), partial(np.quantile, q=0.1111)]}
    )
    ...
    expected_values = np.array(
        [df.resample("3min").A.quantile(q=q).values for q in [0.9998, 0.1111]]
    ).T
    expected = DataFrame(
...
def test_agg_multiple_functions_same_name_with_ohlc_present():
    ...
    result = df.resample("3min").agg(
        {"A": ["ohlc", partial(np.quantile, q=0.9998), partial(np.quantile, q=0.1111)]}
    )
