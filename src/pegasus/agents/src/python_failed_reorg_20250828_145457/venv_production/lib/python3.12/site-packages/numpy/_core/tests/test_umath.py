...
class TestLog:
    ...
    @pytest.mark.parametrize(
        ...
        [(1 + 1e-12j, 5e-25 + 1e-12j),
         (1.000000000000001 + 3e-08j,
          1.5602230246251546e-15 + 2.999899989998996e-08j),
         (0.9998995000000417 + 0.0009998998333333417j,
          7.831475869017683e-18 + 0.001j),
         (0.9998999899989996 + 2.999899989998999e-08j,
          5.9107901499372034e-18 + 3e-08j),
         (0.99985000042 - 0.009998833j,
          -7.015159763822903e-15 - 0.009998999665816696j)],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [(np.complex64(1.0 + 3e-6j), np.complex64(4.5e-12 + 3e-06j)),
         (np.complex64(1.0 - 2e-5j), np.complex64(1.9998999e-10 - 2e-5j)),
         (np.complex64(0.9998999 + 1e-06j),
...
class TestComplexFunctions:
    ...
    def test_loss_of_precision(self, dtype):
        ...
        p = 9.999899989333333333e-6 + 1.000000000066666666e-5j
        d = np.absolute(1 - np.arctanh(z) / p)
        assert_(np.all(d < 1e-15))

        p = 1.0000000000333333333e-5 + 9.999899989666666667e-6j
        d = np.absolute(1 - np.arcsinh(z) / p)
        assert_(np.all(d < 1e-15))

        p = 9.999899989333333333e-6j + 1.000000000066666666e-5
        d = np.absolute(1 - np.arctan(z) / p)
        assert_(np.all(d < 1e-15))

        p = 1.0000000000333333333e-5j + 9.999899989666666667e-6
