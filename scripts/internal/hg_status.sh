...
if port_listening 9998; then
    ...
    [ -z "$PROXY_PID" ] && PROXY_PID=$(listener_pid 9998)
    [ -z "$PROXY_PID" ] && PROXY_PID="unknown"
    echo -e "Proxy:     ${GREEN}✓ HTTP RUNNING${NC} (PID: $PROXY_PID, Port: 9998)"
    if curl -s http://127.0.0.1:9998/hg/telemetry >/dev/null 2>&1; then
        echo -e "           ${GREEN}✓ Responding to requests${NC}"
        TELEMETRY=$(curl -s http://127.0.0.1:9998/hg/telemetry 2>/dev/null)
        if [ -n "$TELEMETRY" ]; then
...
if port_listening 42110; then
    ...
    if curl -fsS --max-time 3 http://127.0.0.1:42110/api/health >/dev/null 2>&1; then
        ...
        # Get stats via proxy
        KHOJ_STATS=$(curl -s http://127.0.0.1:9998/hg/khoj/status 2>/dev/null)
        if [ $? -eq 0 ]; then
