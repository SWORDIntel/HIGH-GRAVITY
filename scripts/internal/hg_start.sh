...
HG_PROXY_WATCHDOG="${HG_PROXY_WATCHDOG:-0}"
HG_PROXY_HTTPS_PORT="${HG_PROXY_HTTPS_PORT:-443}"
HG_PROXY_INTERNAL_HTTPS_PORT="${HG_PROXY_INTERNAL_HTTPS_PORT:-9443}"
HG_CLIENT_TARGET="${HG_CLIENT_TARGET:-antigravity}"
HG_TRAFFIC_MUTATION_ENABLED="${HG_TRAFFIC_MUTATION_ENABLED:-0}"
HG_DECRYPTED_TRAFFIC_LOG="${HG_DECRYPTED_TRAFFIC_LOG:-1}"
HG_DECRYPTED_TRAFFIC_FULL_BODY="${HG_DECRYPTED_TRAFFIC_FULL_BODY:-1}"
HG_KHOJ_BINARY_INJECT="${HG_KHOJ_BINARY_INJECT:-0}"
...
HG_TOKEN_SAVER="${HG_TOKEN_SAVER:-0}"
HG_TOKEN_SAVER_DISABLE_CONTEXT_INJECTION="${HG_TOKEN_SAVER_DISABLE_CONTEXT_INJECTION:-1}"
HG_TOKEN_SAVER_FORCE_LOW_REASONING="${HG_TOKEN_SAVER_FORCE_LOW_REASONING:-0}"
HG_EXACT_RESPONSE_CACHE="${HG_EXACT_RESPONSE_CACHE:-0}"
...
HG_CANONICAL_RESPONSE_CACHE="${HG_CANONICAL_RESPONSE_CACHE:-0}"
HG_CANONICAL_RESPONSE_CACHE_MIN_TEXT_CHARS="${HG_CANONICAL_RESPONSE_CACHE_MIN_TEXT_CHARS:-80}"
HG_LOCAL_ACK_TELEMETRY="${HG_LOCAL_ACK_TELEMETRY:-0}"
...
HG_PEGASUS_SWARM_TRIGGER="${HG_PEGASUS_SWARM_TRIGGER:-0}"
...
HG_PROXY_LOG_DETAIL="${HG_PROXY_LOG_DETAIL:-compact}"
HG_PROXY_VERBOSE_REQUEST_LOGS="${HG_PROXY_VERBOSE_REQUEST_LOGS:-0}"
HG_PROXY_LOG_DEEP_INTEL="${HG_PROXY_LOG_DEEP_INTEL:-0}"
HG_PROXY_LOG_PULSE="${HG_PROXY_LOG_PULSE:-0}"
HG_PROXY_LOG_ACCESS="${HG_PROXY_LOG_ACCESS:-0}"
HG_PROXY_LOG_TO_STDOUT="${HG_PROXY_LOG_TO_STDOUT:-0}"
HG_PROXY_LOG_MAX_BYTES="${HG_PROXY_LOG_MAX_BYTES:-10485760}"
HG_PROXY_LOG_BACKUP_COUNT="${HG_PROXY_LOG_BACKUP_COUNT:-3}"
HG_PROXY_METRICS_JSONL="${HG_PROXY_METRICS_JSONL:-0}"
...
HG_MICROPROXY_FRONT_EVENTS="${HG_EDGE_EVENT_LOG:-$SCRIPT_DIR/logs/microproxy_events.jsonl}"
HG_EDGE_EVENT_LOG="$HG_MICROPROXY_FRONT_EVENTS"
...
HG_LEGACY_IDE_WRAPPER="/usr/local/bin/hg-windsurf-next"
...
ITEMS=(
    ...
    "Start all           Antigravity observe-only proxy stack"
...
draw_menu() {
    ...
    printf "  ${D}Proxy:${NC} $p_s  ${D}Khoj:${NC} $k_s  ${D}Legacy IDE:${NC} $w_s\n\n"
...
_start_proxy_https() {
    ...
    local proxy_log_env
    local khoj_token="${HG_KHOJ_TOKEN:-}"
    rotate_log_if_large "$SCRIPT_DIR/logs/proxy_https.log" "$HG_PROXY_LOG_MAX_BYTES" "$HG_PROXY_LOG_BACKUP_COUNT"
    proxy_log_env="HG_PROXY_LOG_DETAIL=\"$HG_PROXY_LOG_DETAIL\" HG_PROXY_VERBOSE_REQUEST_LOGS=\"$HG_PROXY_VERBOSE_REQUEST_LOGS\" HG_PROXY_LOG_DEEP_INTEL=\"$HG_PROXY_LOG_DEEP_INTEL\" HG_PROXY_LOG_PULSE=\"$HG_PROXY_LOG_PULSE\" HG_PROXY_LOG_ACCESS=\"$HG_PROXY_LOG_ACCESS\" HG_PROXY_LOG_TO_STDOUT=\"$HG_PROXY_LOG_TO_STDOUT\" HG_PROXY_LOG_MAX_BYTES=\"$HG_PROXY_LOG_MAX_BYTES\" HG_PROXY_LOG_BACKUP_COUNT=\"$HG_PROXY_LOG_BACKUP_COUNT\" HG_PROXY_METRICS_JSONL=\"$HG_PROXY_METRICS_JSONL\""
...
_install_legacy_ide_wrapper() {
    local wrapper_src="$SCRIPT_DIR/scripts/internal/hg_windsurf_next.sh"
    if [ -f "$wrapper_src" ]; then
        echo "$SUDO_PASS" | sudo -S install -m 0755 "$wrapper_src" "$HG_LEGACY_IDE_WRAPPER"
        echo "$SUDO_PASS" | sudo -S sed -i \
            -e "s|^Exec=/usr/share/windsurf-next/windsurf-next --open-url %U|Exec=$HG_LEGACY_IDE_WRAPPER --open-url %U|" \
            -e "s|^Exec=/usr/share/windsurf-next/windsurf-next --new-window %F|Exec=$HG_LEGACY_IDE_WRAPPER --new-window %F|" \
            -e "s|^Exec=/usr/share/windsurf-next/windsurf-next %F|Exec=$HG_LEGACY_IDE_WRAPPER %F|" \
...
_launch_windsurf() {
    ...
    nohup sudo -u "$TARGET_USER" env HOME="$TARGET_HOME" "$HG_LEGACY_IDE_WRAPPER" > "$SCRIPT_DIR/logs/windsurf_launch.log" 2>&1 &
...
_launch_watchdog_processes() {
    ...
    if [ -n "$pid" ] && ps -p "$pid" -o pid= >/dev/null 2>&1; then
        ...
        (
            ...
            export HG_CLIENT_TARGET HG_EDGE_EVENT_LOG HG_TRAFFIC_MUTATION_ENABLED HG_DECRYPTED_TRAFFIC_LOG HG_DECRYPTED_TRAFFIC_FULL_BODY
            export HG_KHOJ_BINARY_INJECT HG_KHOJ_BINARY_INLINE_TIMEOUT_SECONDS HG_BINARY_REASONING_INJECT_MAX_BYTES
            export HG_TOKEN_SAVER HG_TOKEN_SAVER_DISABLE_CONTEXT_INJECTION HG_TOKEN_SAVER_FORCE_LOW_REASONING
            ...
            export HG_PROXY_LOG_DETAIL HG_PROXY_VERBOSE_REQUEST_LOGS HG_PROXY_LOG_DEEP_INTEL HG_PROXY_LOG_PULSE
            export HG_PROXY_LOG_ACCESS HG_PROXY_LOG_TO_STDOUT HG_PROXY_LOG_MAX_BYTES HG_PROXY_LOG_BACKUP_COUNT HG_PROXY_METRICS_JSONL
...
do_start_proxy() {
    ...
    if [ "$HG_PROXY_WATCHDOG" = "1" ] || [ "$HG_PROXY_WATCHDOG" = "true" ]; then
...
do_start() {
    ...
    if [ "${HG_CLIENT_TARGET:-antigravity}" != "antigravity" ]; then
        ...
        _stop_windsurf
        # Legacy IDE launch is retained only outside Antigravity target mode.
        _launch_windsurf
    else
        echo -e "${D}  [~] Antigravity mode: skipping legacy IDE shim/wrapper and launch${NC}"
    fi
