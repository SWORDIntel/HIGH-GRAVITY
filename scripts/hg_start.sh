...
# Clean repatch:      bash scripts/repatch.sh    (or menu item 2)
...
iptables_active() {
    echo "$SUDO_PASS" | sudo -S iptables -t nat -C OUTPUT -p tcp --dport 50001 -j REDIRECT --to-port 9999 >/dev/null 2>&1
}
pidfile_read() {
    local pidfile="$1"
    [ -f "$pidfile" ] || return 1
...
_start_proxy_http() {
    mkdir -p logs
    local root_cmd
    root_cmd="cd \"$SCRIPT_DIR\" && export PYTHONNOUSERSITE=1 PYTHONPATH=\"$SCRIPT_DIR\" && nohup \"$PROXY_PYTHON_BIN\" \"$SCRIPT_DIR/src/proxy.py\" >> \"$SCRIPT_DIR/logs/proxy.log\" 2>&1 & echo \$! > \"$SCRIPT_DIR/logs/proxy.pid\""
    echo "$SUDO_PASS" | sudo -S bash -lc "$root_cmd"
}

_start_proxy_https() {
    mkdir -p logs
    local root_cmd
    root_cmd="cd \"$SCRIPT_DIR\" && export PYTHONNOUSERSITE=1 PYTHONPATH=\"$SCRIPT_DIR\" && nohup \"$PROXY_PYTHON_BIN\" \"$SCRIPT_DIR/src/proxy.py\" --https >> \"$SCRIPT_DIR/logs/proxy_https.log\" 2>&1 & echo \$! > \"$SCRIPT_DIR/logs/proxy_https.pid\""
    echo "$SUDO_PASS" | sudo -S bash -lc "$root_cmd"
}

_start_khoj_async() {
    mkdir -p logs
    if [ -f "scripts/khoj_docker.sh" ]; then
        nohup bash scripts/khoj_docker.sh >> logs/khoj_docker.log 2>&1 &
        echo $! > logs/khoj_docker.pid
    fi
}
...
_watchdog_khoj() {
    while true; do
        sleep 15
        curl -s http://127.0.0.1:42110/api/health 2>/dev/null | grep -q OK && continue
        echo "$(date): khoj health down, restarting docker launcher" >> "$SCRIPT_DIR/logs/proxy_watchdog.log"
        _start_khoj_async
    done
}
...
do_start() {
    ...
    bash scripts/khoj_stop.sh >/dev/null 2>&1 || true
    docker stop khoj khoj-pg >/dev/null 2>&1 || true
    rm -f logs/proxy.pid logs/proxy_https.pid logs/proxy_watchdog.pid logs/khoj_watchdog.pid logs/khoj_docker.pid
    sleep 1
    echo -e "${G}  [+] Cleaned${NC}"

    mkdir -p logs

    # ── iptables: redirect port 50001 → 9999 (language server fallback) ──
    echo "$SUDO_PASS" | sudo -S iptables -t nat -C OUTPUT -p tcp --dport 50001 -j REDIRECT --to-port 9999 2>/dev/null \
        || echo "$SUDO_PASS" | sudo -S iptables -t nat -A OUTPUT -p tcp --dport 50001 -j REDIRECT --to-port 9999
    echo -e "${G}  [+] iptables 50001→9999 active${NC}"

    # ── HTTP proxy (always) ───────────────────────────────────────────
    echo -e "${B}  [*] Starting HTTP proxy (9999)${NC}"
    _start_proxy_http
    if wait_for_port 9999 15 1; then
    ...
    if [ -f "certs/proxy.crt" ] && [ -f "certs/proxy.key" ]; then
        ...
        echo -e "${D}     (run: bash scripts/hg_start.sh from menu to generate)${NC}"
    fi
    ...
    if [ -f "scripts/khoj_docker.sh" ]; then
        ...
        if curl -s http://127.0.0.1:42110/api/health 2>/dev/null | grep -q OK; then
            ...
            curl -s -X POST http://127.0.0.1:9999/hg/khoj/reindex >/dev/null 2>&1 \
                && echo -e "${D}  [~] Khoj reindex triggered${NC}"
        else
            echo -e "${Y}  [~] Khoj warming up in background (logs/khoj_docker.log)${NC}"
        fi
    else
        echo -e "${D}  [~] scripts/khoj_docker.sh not found, skipping${NC}"
    fi
...
do_verify_inline() {
    ...
    # ── Services ─────────────────────────────────────────────────────
    check_port 9999 && echo -e "  Proxy HTTP  (9999)  ${G}UP${NC}" \
        || echo -e "  Proxy HTTP  (9999)  ${R}DOWN${NC}"
    check_port 443  && echo -e "  Proxy HTTPS (443)   ${G}UP${NC}" \
        || echo -e "  Proxy HTTPS (443)   ${Y}DOWN${NC}"
    curl -s http://127.0.0.1:42110/api/health 2>/dev/null | grep -q OK \
        && echo -e "  Khoj        (42110) ${G}UP${NC}" \
        || echo -e "  Khoj        (42110) ${D}--${NC}"

    # ── iptables ─────────────────────────────────────────────────────
    if iptables_active; then
        echo -e "  iptables    50001→9999  ${G}ACTIVE${NC}"
    else
        echo -e "  iptables    50001→9999  ${R}MISSING${NC}"
    fi
...
do_dashboard() {
    ...
    elif [ -f "archive/old_scripts/hg_dashboard.py" ]; then
        exec python3 archive/old_scripts/hg_dashboard.py
    else
        printf "${CLR}"
...
do_tidy() {
    ...
    for f in setup_network_redirect.sh start_https_proxy.sh launch_debug.sh \
        ...
        if [ -f "$f" ]; then
            ...
            echo -e "  ${G}>${NC} $f → archive/old_scripts/"
            moved=$((moved+1))
        fi
    done
